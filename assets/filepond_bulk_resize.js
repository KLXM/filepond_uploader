...
$(document).on('rex:ready', function (event, element) {
    ...
    let $table = $('#filepond-bulk-resize-table');
    ...
    function startBatchProcessing(filenames) {
        ...
        // Hole aktuelle Einstellungen aus Formular
        let maxWidth = parseInt($('#max_width').val()) || 0;
        let maxHeight = parseInt($('#max_height').val()) || maxWidth;
        let quality = parseInt($('#quality').val()) || 85;

        // Starte Batch
        $.ajax({
            url: window.location.pathname + '?rex-api-call=filepond_bulk_resize',
            method: 'POST',
            dataType: 'json',
            data: {
                action: 'start',
            ...
            success: function(response) {
                if (response.batch_id) {
                    batchProcessor = new BatchProcessor(response.batch_id, response.batch);
                    batchProcessor.start();
                } else {
                    hideProgressModal();
                    alert('Fehler beim Starten der Verarbeitung: ' + (response.error || 'Unbekannter Fehler'));
                }
            },
                        ...
                        <div class="modal-body" style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); padding: 30px;">
                            ...
                            <div id="results-section" style="display: none;">
                                ...
                                <div style="margin-top: 15px; padding: 15px; background: white; border-radius: 8px;">
                                    <div style="font-size: 14px; color: #666;">
                                        Gespart: <span id="saved-bytes" style="font-weight: bold; color: #28a745;">0 KB</span>
                                    </div>
                                </div>
                            </div>
    ...
    function updateProgressModal(batchStatus) {
        ...
        if ((batchStatus.processed || 0) > 0) {
            ...
            $('#saved-bytes').text(batchStatus.savedBytesFormatted || '0 KB');
        }
    ...
    class BatchProcessor {
        ...
        processNext() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.batch) {
                        this.status = response.batch;
                        updateProgressModal(this.status);

                        if (this.status.status === 'completed' || response.status === 'completed') {
                            this.running = false;
                        } else if (response.status === 'processing' || this.status.status === 'running') {
                            // Schnelleres Polling für parallele Verarbeitung (200ms)
                            this.processInterval = setTimeout(() => {
                                this.processNext();
                            }, 200);
                        } else {
                            this.handleError('Unerwarteter Status: ' + (response.status || this.status.status));
                        }
                    } else if (response.error) {
                        this.handleError('Verarbeitungsfehler: ' + response.error);
                    } else {
                        this.handleError('Unbekannte Antwort vom Server');
                    }
                },
        ...
        handleError(message) {
            this.running = false;
            $('#batch-status-text').removeClass().addClass('badge badge-danger').text('Fehler');
    ...
    $(document).on('click', '#cancel-batch', function() {
        if (batchProcessor) {
            batchProcessor.stop();
            $('#batch-status-text').removeClass().addClass('badge badge-warning').text('Abgebrochen');
            $('#cancel-batch').hide();
            $('#close-modal').show();
        }
    });
